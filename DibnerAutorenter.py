...
roomList = {}  # match-up of rooms and available hours
roomHourToID = {}  # match-up of room+hour to html ID
...
# takes user chosen date and clicks on table to date. Note: cannot select month
while True:
...
# sanity check for starting time
while True:
...
# populate userTimes based on starting hour and number of hours
for i in range(hours):
    userTimes.append(roomTimes[i+startingIndex])

elem = driver.find_elements_by_class_name("lc_rm_a")

# read html from website and populate array with room and hours
for option in elem:
    singleRoomData = option.get_attribute("title")
    singleRoomID = option.get_attribute("id")
    singleRoomData = singleRoomData.replace(',','')
...
# check available rooms and compare to userTimes for valid room, then grab id
